...
fun Route.student() =
    route(studentsPath) {
        ...
        get("{id}") {
            ...
            call.response.etag("djhgcdksf=${studentItem.etag.toString()}")
...
fun Route.group() =
    route(groupsPath) {
        ...
        get("{groop}") {
            val id = call.parameters["groop"] ?: return@get call.respondText(
                "Missing or malformed id",
                status = HttpStatusCode.BadRequest
            )
            val studentItem =
                studentsRepo.find { it.groop == id }
            call.response.etag(studentItem.toString())
            call.respond(studentItem)
        }
    }